...
window.addEventListener("load", async function () {
    ...
    addPrescriptionBtn.addEventListener("click", async function () {
        if ( presDate.value && addprescriptionPic.files[0]) {
            let obj = {};
            let link;
            let t = Date.now();
            ...
            task.on('state_changed', function (snapshot) {
                ...
                function () {
                    storage.ref("images/prescription").child(`${t}${addprescriptionPic.files[0].name}`).getDownloadURL().then(async function (url) {
                        ...
                        obj.pid = pid;
                        console.log(obj);
                        await axios.post("https://horizoncenter.herokuapp.com/doctor/addPres",obj);
...
async function getPres() {
    let data = await axios.post("https://horizoncenter.herokuapp.com/doctor/getPres", {pid:pid});
